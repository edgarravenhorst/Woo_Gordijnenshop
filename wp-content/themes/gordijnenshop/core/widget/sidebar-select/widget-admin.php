<div id='<?= $this->id ?>'>
	<p>
		<label for="<?php echo $this->get_field_id('count'); ?>">Aantal projecten:
	...
	<p>
		...
		<select name="<?php echo $this->get_field_name('category'); ?>" id="<?php echo $this->get_field_id('category'); ?>" class="widefat">
			...
			<?php
			$cats = get_terms('tax_project');
			foreach ($cats as $cat) {

				if($category == $cat->slug)
					echo '<option selected value="' . $cat->name . '" id="' . $cat->name. '">'. $cat->name . '</option>';
				else
					echo '<option value="' . $cat->name . '" id="' . $cat->name. '">'. $cat->name . '</option>';
